...
import { triggerModelUpdate } from "./ChatModelInfo";
...
const SidebarComponent = ({ children, onNewChat, getCurrentChatInfo }: SidebarProps) => {
  ...
  const handleReset = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      const response = await fetch("/api/openrouter/clear-cache", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        console.log("[Reset] Cache cleared:", data);
        // Показываем уведомление с информацией о провайдере и модели
        const provider = data.default_provider || "openrouter";
        const model = data.default_model || "mistralai/devstral-2512:free";
        alert(`ПОЛНЫЙ СБРОС выполнен. Все кэши очищены (Redis + PostgreSQL). Провайдер: ${provider}, Модель: ${model}`);
        
        // Ждем немного, чтобы Redis успел сохранить данные, затем обновляем информацию о модели в UI
        // Делаем несколько попыток обновления с задержками для гарантии
        setTimeout(() => {
          console.log("[Reset] Первое обновление информации о модели...");
          triggerModelUpdate();
        }, 300); // Первая попытка через 300ms
        
        setTimeout(() => {
          console.log("[Reset] Второе обновление информации о модели...");
          triggerModelUpdate();
        }, 1000); // Вторая попытка через 1 секунду
        
        setTimeout(() => {
          console.log("[Reset] Финальное обновление информации о модели...");
          triggerModelUpdate();
        }, 2000); // Финальная попытка через 2 секунды
      } else {
        const error = await response.text();
        console.error("[Reset] Failed:", error);
        alert("Ошибка при сбросе: " + error);
      }
    } catch (err) {
      console.error("[Reset] Error:", err);
      alert("Ошибка соединения при сбросе кэша");
    } finally {
  ...
  const handleTokens = (e: React.MouseEvent) => {
    e.stopPropagation();
    // Сохраняем текущий путь перед переходом в настройки
    // Это позволит вернуться к тому же чату после закрытия настроек
    const currentPath = window.location.pathname;
    // Сохраняем путь в sessionStorage для восстановления при возврате
    if (currentPath !== "/settings") {
      sessionStorage.setItem("previousPathBeforeSettings", currentPath);
    }
    navigate("/settings");
  };
  ...
  const handleNewChat = () => {
    ...
    if (currentChatInfo) {
      // Если чат уже сохранен в истории (есть имя), НЕ спрашиваем название снова.
      // Просто "пересохраняем" текущий чат под его существующим именем и создаем новый.
      const existingChat = chats.find((c) => c.threadId === currentChatInfo.threadId);
      if (existingChat?.title) {
        // Примечание: передаем existingChat.title как customTitle, чтобы гарантированно
        // не перезаписать имя чата на derived-title из firstMessage.
        saveChat(currentChatInfo.threadId, currentChatInfo.firstMessage, existingChat.title);
        navigate("/");
        onNewChat();
        return;
      }

      // Чат новый (не сохранен), спрашиваем название перед созданием нового
    ...
    <>
      ...
      <div
        className={[
          ...
          "bg-card border text-card-foreground flex flex-col",
      ...
      >
        ...
        <div
          className={`flex items-center p-2 text-sm rounded-lg cursor-pointer hover:bg-white/10 ${isResetting ? "opacity-50 pointer-events-none" : ""}`}
          onClick={handleReset}
        >
          <RotateCcw size={24} className={`mr-2 ${isResetting ? "animate-spin" : ""}`} />
        ...
        {/* Кнопка запуска отложенных задач (только для админа) */}
        {user?.username === "alexis" && (
          <div
            className="flex items-center p-2 text-sm rounded-lg cursor-pointer hover:bg-white/10"
            onClick={() => {
              navigate("/deferred-tasks");
        ...
        {/* Промежуточная область для растягивания - позволяет списку чатов быть внизу */}
        <div className="flex-1 min-h-0 overflow-hidden flex flex-col">
          {/* Список сохраненных чатов - в прокручиваемом контейнере */}
        ...
        {user && (
          <div className="mb-2 p-2 text-xs text-muted-foreground">
