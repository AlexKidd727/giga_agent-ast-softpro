...
const resolveUrlForTransport = (rawUrl: string, forceProxy = false): string => {
  try {
    ...
    // Автоматически добавляем /mcp если путь не указан или пустой
    const path = target.pathname;
    if (!path || path === "/") {
      target.pathname = "/mcp";
      effectiveUrl = target.toString();
    } else if (!path.endsWith("/mcp") && !path.includes("/mcp/")) {
      // Если путь указан, но не содержит /mcp, добавляем его
      // Но только если это не локальный адрес (для локальных адресов пользователь может указать свой путь)
      if (!isLocal) {
        target.pathname = path.endsWith("/") ? `${path}mcp` : `${path}/mcp`;
        effectiveUrl = target.toString();
      }
    }

    // Для локальных адресов возвращаем URL напрямую (без прокси)
    if (isLocal && !forceProxy) {
      console.log("[resolveUrlForTransport] Local address, using direct URL:", effectiveUrl);
      return effectiveUrl;
    }
    ...
    console.log("[resolveUrlForTransport] Remote address, using proxy:", `${proxyUrl}${effectiveUrl}`);
    return `${proxyUrl}${effectiveUrl}`;
  } catch (error) {
    console.error("[resolveUrlForTransport] Error processing URL:", error, rawUrl);
...
const McpServerModal: React.FC<McpServerModalProps> = ({
  ...
  // Локальное состояние серверов MCP
  ...
  // Хелпер: обновление serverTools (без localStorage)
  ...
  // Загрузка MCP серверов и инструментов из БД при монтировании
  useEffect(() => {
    if (!isAuthenticated || !token) {
      // Если пользователь не аутентифицирован, используем пустые значения
      setServers([]);
      setServerTools({});
    ...
    const loadMcpData = async () => {
      try {
        ...
        if (preferences?.mcpServers) {
          setServers(preferences.mcpServers);
        } else {
          setServers([]);
        }
        
        if (preferences?.mcpServerTools) {
          setServerTools(preferences.mcpServerTools);
        } else {
          setServerTools({});
        }
      } catch (error) {
        console.error("Ошибка загрузки MCP данных из БД:", error);
        setServers([]);
        setServerTools({});
      } finally {
        setIsInitialized(true);
  ...
  const [transportType, _setTransportType] = useState<"auto" | "http" | "sse">("auto");
  ...
  // Сохранение MCP серверов в БД при изменении
  useEffect(() => {
    ...
    const timeoutId = setTimeout(async () => {
      try {
        ...
        console.error("Ошибка сохранения MCP серверов в БД:", error);
      }
    }, 500); // Задержка 500мс для debounce

    return () => clearTimeout(timeoutId);
  }, [servers, isInitialized, isAuthenticated, token]);

  // Сохранение MCP инструментов в БД при изменении
  useEffect(() => {
    // Не сохраняем, если еще не инициализированы или пользователь не аутентифицирован
    if (!isInitialized || !isAuthenticated || !token) {
      return;
    ...
    const timeoutId = setTimeout(async () => {
      try {
        await updateUserPreferences(token, {
          mcpServerTools: serverTools,
        });
      } catch (error) {
        console.error("Ошибка сохранения MCP инструментов в БД:", error);
      }
    }, 500); // Задержка 500мс для debounce

    return () => clearTimeout(timeoutId);
  }, [serverTools, isInitialized, isAuthenticated, token]);
