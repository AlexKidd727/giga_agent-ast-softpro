...
export function useMcp(options: UseMcpOptions): UseMcpResult {
  ...
  const connect = useCallback(async () => {
    ...
    const tryConnectWithTransport = async (
      ...
      addLog(
        "info",
        `Attempting connection with transport: ${transportTypeParam} to URL: ${url}`,
      );
      console.log(`[useMcp] Connecting to MCP server: ${url} with transport: ${transportTypeParam}`);
      ...
      try {
        ...
        console.log(`[useMcp] Server config:`, {
        ...
        console.error("[useMcp] Connection error:", {
          error: err,
        ...
        if (
          ...
          failConnection(
            "CORS ошибка: Сервер MCP не разрешает подключение из браузера. " +
              "Проверьте настройки CORS на сервере или используйте прокси.",
          );
          return "failed";
        }
        ...
        if (
          ...
          failConnection(
            `Ошибка сети: Не удалось подключиться к MCP серверу по адресу ${url}. ` +
              `Возможные причины: сервер недоступен, проблемы с сетью, или настройки CORS на сервере MCP блокируют запрос. ` +
              `Попробуйте использовать прокси, если сервер находится в другой сети.`,
            err,
          );
          return "failed";
        }
        ...
        if (
          ...
          failConnection(
            `Ошибка сети: Не удалось подключиться к серверу по адресу ${url}. ` +
              `Проверьте доступность сервера и правильность URL.`,
            err,
          );
          return "failed";
        }
        ...
        if (
          ...
          failConnection(
            "Таймаут подключения: Сервер не отвечает в течение установленного времени. " +
              "Проверьте доступность сервера и увеличьте timeout, если необходимо.",
          );
          return "failed";
        }

        // Handle "Could not connect with any available transport" error
        if (
          errorMessage.includes("Could not connect to server with any available transport")
        ) {
          const httpError = (err as any)?.httpError;
          const sseError = (err as any)?.sseError;
          let detailedMessage = errorMessage;
          
          // Проверяем специфические ошибки
          const httpErrorMsg = httpError?.message || String(httpError || "");
          const sseErrorMsg = sseError?.message || String(sseError || "");
          
          // Ошибка валидации Pydantic (422) - сервер ожидает строку для id, но получает число
          if (httpErrorMsg.includes("422") || httpErrorMsg.includes("string_type") || httpErrorMsg.includes("Input should be a valid string")) {
            detailedMessage = 
              `Ошибка подключения: Сервер MCP не принимает формат JSON-RPC запросов. ` +
              `Сервер ожидает, что поле 'id' будет строкой, но SDK отправляет число. ` +
              `Это проблема конфигурации сервера MCP - он должен принимать 'id' как строку, число или null согласно спецификации JSON-RPC 2.0. ` +
              `Обратитесь к администратору сервера MCP для исправления валидации.`;
          }
          // Ошибка 405 - метод не поддерживается
          else if (sseErrorMsg.includes("405") || sseErrorMsg.includes("Method Not Allowed")) {
            detailedMessage = 
              `Ошибка подключения: Сервер MCP не поддерживает SSE (Server-Sent Events) транспорт. ` +
              `Попробуйте использовать только HTTP транспорт в настройках сервера. ` +
              `HTTP транспорт также не удался: ${httpErrorMsg || "неизвестная ошибка"}`;
          }
          // Общая ошибка с деталями
          else if (httpError || sseError) {
            detailedMessage = 
              `Не удалось подключиться к серверу MCP. ` +
              `HTTP транспорт: ${httpErrorMsg || "неизвестная ошибка"}. ` +
              `SSE транспорт: ${sseErrorMsg || "неизвестная ошибка"}. ` +
              `Проверьте: доступность сервера, правильность URL (должен заканчиваться на /mcp), настройки CORS, наличие токена аутентификации если требуется.`;
            
            if (httpError) {
              console.error("[useMcp] HTTP transport error:", httpError);
            }
            if (sseError) {
              console.error("[useMcp] SSE transport error:", sseError);
            }
          }
          
          failConnection(detailedMessage, err);
          return "failed";
        }
