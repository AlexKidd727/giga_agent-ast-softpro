  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        // Собираем детали ошибок для более информативного сообщения
        const httpErrorMsg = fallbackReason || "Unknown HTTP error";
        const sseErrorMsg = sseErr?.message || String(sseErr || "Unknown error");
        
        const detailedError = new Error(
          `Could not connect to server with any available transport. ` +
          `HTTP transport error: ${httpErrorMsg}. ` +
          `SSE transport error: ${sseErrorMsg}. ` +
          `Server URL: ${baseUrl}. ` +
