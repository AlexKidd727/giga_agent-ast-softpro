...
export function useRag(): UseRagReturn {
  ...
  const getCollections = useCallback(
    async (accessToken?: string): Promise<Collection[]> => {
      if (!session?.accessToken && !accessToken) {
        console.error("🔍 RAG getCollections: accessToken не найден");
        toast.error("Не удалось подключиться к LangConnect API", {
          richColors: true,
          description: "Не удалось получить коллекции. Проверьте настройки VITE_LANGCONNECT_API_SECRET_TOKEN.",
        });
        return [];
      }
      ...
      try {
        ...
        console.log("🔍 RAG getCollections: Запрос к", url.toString());
        ...
        console.log("🔍 RAG getCollections: Ответ сервера", response.status, response.statusText);
        
        if (!response.ok) {
          const errorText = await response.text();
          console.error("🔍 RAG getCollections: Ошибка ответа", errorText);
          throw new Error(`Failed to fetch collections: ${response.statusText} (${response.status})`);
        }
        const data = await response.json();
        console.log("🔍 RAG getCollections: Получено коллекций", data?.length || 0);
        return data;
      } catch (error: any) {
        console.error("🔍 RAG getCollections: Исключение", error);
        throw error;
      }
    },
    [session],
