    ...
    return (
      ...
      >
        ...
        {messages.map((message, idx) =>
          message.type === "tool" ? (
            // Скрываем промежуточные шаги (tool messages) когда отладка выключена
            // Также скрываем результаты поисковых инструментов (search, get_urls, researcher_agent) - это рабочая информация
            (() => {
              ...
              // Список поисковых инструментов, результаты которых не нужно показывать
              // Это рабочая информация, которая не должна отображаться в чате
              const searchTools = ["search", "get_urls", "researcher_agent"];
              const toolNameLower = toolName ? toolName.toLowerCase() : "";
              const isSearchTool = searchTools.some(tool => 
                toolNameLower === tool.toLowerCase() || 
                toolNameLower.includes(tool.toLowerCase())
              );
              
              // Показываем только если включена отладка И это не поисковый инструмент
              if (settings.debugMode && !isSearchTool) {
                return (
                  <ToolMessage
                    key={idx}
                    message={message}
              ...
              // Скрываем поисковые инструменты и tool messages когда отладка выключена
              return null;
            })()
          ) : (
            <Message
        ...
        {/* Скрываем ToolExecuting когда отладка выключена */}
        {settings.debugMode && (
