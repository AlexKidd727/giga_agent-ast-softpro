...
const MessageEditor: React.FC<MessageEditorProps> = ({
  ...
  const handleSendMessage = useCallback(async () => {
    ...
    if (!isAuthenticated || !userId) {
      console.error("❌ Невозможно отредактировать сообщение: пользователь не аутентифицирован");
      return;
    }
    ...
    if (!configurable.user_id || configurable.user_id.trim().toLowerCase() === 'anonymous') {
      console.error(`❌ Невозможно отредактировать сообщение: невалидный user_id: '${configurable.user_id}'`);
      return;
    }
