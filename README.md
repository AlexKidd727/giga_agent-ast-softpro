...
!!! пока изменена только ветка DEV
...
- работать с разными моделями, [доступными в LangChain](https://python.langchain.com/docs/integrations/chat/#all-chat-models): GigaChat, ChatGPT, DeepSeek, Anthropic и другими
...
- синтезировать речь через Sber SmartSpeech (salute_say)
- работать с Google Calendar для управления событиями
- работать с Tinkoff Invest API для торговли и анализа портфеля
- работать с кодом через специализированного coder-агента
...
В этой версии добавлены следующие субагенты:

* **tinkoff_agent** — работа с Tinkoff Invest API (торговля, анализ портфеля, графики)
* **calendar_agent** — управление Google Calendar (создание, просмотр, обновление событий)
* **coder_agent** — специализированный агент для работы с кодом
* **email_agent** — работа с электронной почтой (чтение, отправка, фильтрация)

> [!NOTE]
> **pc_agent** временно отключен в текущей версии.
...
* `GIGA_AGENT_LLM="deepseek-reasoner"` — для выбора модели DeepSeek Reasoner в качестве основной
* `GIGA_AGENT_LLM="deepseek-chat"` — для выбора модели DeepSeek Chat в качестве основной

По такому же принципу выбирается модель для векторного представления текстов `GIGA_AGENT_EMBEDDINGS`.

#### DeepSeek

Для работы с моделями DeepSeek необходимо установить переменную окружения:

* **DEEPSEEK_API_KEY** — API ключ DeepSeek (можно получить на https://platform.deepseek.com/)

Поддерживаемые модели DeepSeek:
* `deepseek-reasoner` — модель с режимом размышлений (reasoning mode)
* `deepseek-chat` — стандартная чат-модель

Пример заполнения:
* `GIGA_AGENT_LLM="deepseek-reasoner"` — для использования DeepSeek Reasoner
* `GIGA_AGENT_LLM_FAST="deepseek-chat"` — для использования быстрой модели DeepSeek Chat
...
Тулы/агенты, которые зависят от сервиса: **podcast_generate**, **salute_say**

**salute_say** — инструмент для быстрого синтеза речи через Sber SmartSpeech. Позволяет озвучивать текст с выбором голоса и формата аудио (wav16, ogg, mp3).

Для работы необходимо указать:
* **SALUTE_SPEECH** — базовый токен для доступа к SmartSpeech API
* **SALUTE_SPEECH_SCOPE** (опционально) — область доступа, по умолчанию `SALUTE_SPEECH_PERS`
* **SBER_TTS_TIMEOUT** (опционально) — таймаут запросов, по умолчанию 30 секунд

Получить API ключ можно здесь: https://developers.sber.ru/portal/products/smartspeech
...
Тулы/агенты, которые зависят от сервиса: **tinkoff_agent**

**tinkoff_agent** — субагент для работы с Tinkoff Invest API. Позволяет:
- Просматривать портфель и позиции
- Размещать рыночные и лимитные ордера
- Получать информацию об инструментах
- Анализировать операции
- Создавать графики по тикерам

Для работы необходимо указать:
* **TINKOFF_TOKEN** — токен доступа к Tinkoff Invest API
* **TINKOFF_SANDBOX** (опционально) — использовать песочницу, по умолчанию `false`
* **TINKOFF_MAX_ORDER_AMOUNT** (опционально) — максимальная сумма ордера, по умолчанию 100000
* **TINKOFF_MIN_ORDER_AMOUNT** (опционально) — минимальная сумма ордера, по умолчанию 1000

Получить токен можно здесь: https://www.tinkoff.ru/invest/settings/api/

> [!NOTE]
> Токен Tinkoff также можно настроить через интерфейс пользователя в разделе управления токенами. Это позволяет каждому пользователю использовать свой токен.

#### Google Calendar

Тулы/агенты, которые зависят от сервиса: **calendar_agent**

**calendar_agent** — субагент для работы с Google Calendar через Service Account. Позволяет:
- Создавать события
- Просматривать список событий
- Обновлять и удалять события
- Находить свободные слоты
- Искать события по ключевым словам

Для работы необходимо:
1. Создать Service Account в Google Cloud Console
2. Скачать JSON файл с ключами
3. Поместить файл в папку `credentials/` (например, `service-account.json`)
4. Указать переменные окружения:
   * **GOOGLE_CALENDAR_CREDENTIALS** — путь к JSON файлу Service Account (для Docker: `/app/credentials/service-account.json`, для локального запуска: `./credentials/service-account.json`)
   * **CALENDAR_ID** (опционально) — ID календаря, по умолчанию `primary`

Подробная инструкция по настройке: [credentials/README.md](credentials/README.md)

> [!NOTE]
> Credentials Google Calendar также можно настроить через интерфейс пользователя в разделе управления токенами. Это позволяет каждому пользователю использовать свой Service Account.
...
- **SubAgents** — субагенты, выполняющие специфические задачи: создание презентаций, создание лендингов, работа с Tinkoff Invest, управление Google Calendar, работа с кодом, работа с email и другие
...
- **LLM Tools** — инструменты (функции) для моделей: поиск, работа с данными ВК, работа с github, синтез речи (salute_say) и другие
