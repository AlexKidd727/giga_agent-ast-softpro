...
def get_installed_programs() -> List[str]:
    """Получает список установленных программ из реестра и меню Пуск"""
    programs = []
    ...
    try:
        ...
        # Получение программ из реестра (HKEY_LOCAL_MACHINE и HKEY_CURRENT_USER)
        try:
            import winreg
            
            # Список путей в реестре для поиска установленных программ
            registry_paths = [
        ...
        # Получение программ из меню Пуск (Start Menu)
        try:
            start_menu_paths = [
                        ...
                        for root, dirs, files in os.walk(start_menu_path):
                            for file in files:
                                if file.endswith('.lnk'):
                                    # Извлекаем имя программы из ярлыка
            ...
            logger.warning(f"Ошибка получения программ из меню Пуск: {e}")
...
def get_available_drives() -> List[str]:
    ...
    try:
        ...
        if IS_WINDOWS:
            # Для Windows используем psutil для получения дисков
            if PSUTIL_AVAILABLE:
                for partition in psutil.disk_partitions():
                    try:
                        # Проверяем, что диск доступен для чтения
                        if os.path.exists(partition.mountpoint):
                            drives.append(partition.mountpoint)
                    except (PermissionError, OSError):
                        continue
            else:
                # Fallback: используем стандартный способ для Windows
                import string
            ...
            # Для Linux/Unix используем корневую директорию
            if os.path.exists("/"):
                drives.append("/")
...
def is_safe_path(file_path: str) -> bool:
    ...
    try:
        ...
        # КРИТИЧЕСКИ ВАЖНО: Добавляем папку с файлами пользователей (FILES_DIR)
        # Это позволяет читать файлы из папки пользователя
        files_dir = os.environ.get("FILES_DIR", "files")
        if files_dir:
            files_dir_abs = os.path.abspath(files_dir)
            if files_dir_abs not in safe_dirs:
                safe_dirs.append(files_dir_abs)
...
def is_system_directory(path: str) -> bool:
    """Проверяет, является ли директория системной (для исключения из глобального поиска)"""
    try:
        abs_path = os.path.abspath(path).lower()
        
        # Список системных директорий для исключения
        system_dirs = [
            'windows', 'system32', 'syswow64', 'program files', 'program files (x86)',
            'programdata', '$recycle.bin', 'system volume information',
            'boot', 'recovery', 'perflogs', 'msocache', 'config.msi',
            'appdata\\local\\temp', 'appdata\\local\\microsoft',
            'appdata\\roaming\\microsoft', 'appdata\\local\\temp',
