...
try:
    ...
    from t_tech.invest.schemas import OrderDirection, OrderType, InstrumentType
    # Для совместимости создаем алиас
    OperationType = OrderDirection
    TINKOFF_AVAILABLE = True
    logger.info("tinkoff_client.py: используется t_tech.invest (t-tech-investments)")
except ImportError:
    try:
        ...
        OperationType = OrderDirection
        TINKOFF_AVAILABLE = True
        logger.info("tinkoff_client.py: используется tinkoff.invest")
    except ImportError:
        try:
            ...
            TINKOFF_AVAILABLE = True
            logger.info("tinkoff_client.py: используется tinkoff_invest")
        except ImportError:
            ...
            Operation = None
            logger.warning("tinkoff_client.py: Tinkoff SDK недоступен (пакет не установлен)")
...
class TinkoffClient:
    ...
    def __init__(self, account_id: Optional[str] = None, token: Optional[str] = None, sandbox: Optional[bool] = None):
        if not TINKOFF_AVAILABLE:
            raise ImportError("Tinkoff SDK не установлен (нужен t-tech-investments или совместимый пакет)")
    ...
    async def get_instrument_by_figi(self, figi: str) -> Optional[Dict[str, Any]]:
        ...
        # Справочник известных валютных FIGI (API может возвращать ошибку для них)
        KNOWN_CURRENCIES = {
            "USD800UTSTOM": {"ticker": "USD", "name": "Доллар США", "currency": "RUB", "instrument_type": "currency"},
            "RUB000UTSTOM": {"ticker": "RUB", "name": "Российский рубль", "currency": "RUB", "instrument_type": "currency"},
            "BBG0013HJJ31": {"ticker": "EUR_RUB__TOM", "name": "Евро", "currency": "RUB", "instrument_type": "currency"},
            "BBG0013HGFT4": {"ticker": "USD000UTSTOM", "name": "Доллар США", "currency": "RUB", "instrument_type": "currency"},
            "BBG0013HRTL0": {"ticker": "CNY_RUB__TOM", "name": "Китайский юань", "currency": "RUB", "instrument_type": "currency"},
        }
        
        # Проверяем известные валюты сначала
        if figi in KNOWN_CURRENCIES:
            info = KNOWN_CURRENCIES[figi]
            return {
                "figi": figi,
                "ticker": info["ticker"],
                "name": info["name"],
