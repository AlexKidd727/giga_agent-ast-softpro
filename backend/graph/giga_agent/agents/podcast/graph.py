...
async def audio_gen(state: PodcastState, config: RunnableConfig):
    ...
    if not upload_resp or len(upload_resp) == 0:
        raise ValueError("Не удалось загрузить аудио-файл. Проверьте настройки загрузки файлов.")
...
async def podcast_generate(
    ...
    async for chunk in client.runs.stream(
        ...
        if chunk.event == "values" and chunk.data is not None:
            state = chunk.data
        elif chunk.event == "updates" and chunk.data is not None:
            push_ui_message(
                "agent_execution",
                {
                    "agent": "podcast_generate",
                    "node": list(chunk.data.keys())[0] if chunk.data else "__end__",
                },
            )
    ...
    if not state:
        return {
            "transcript": "",
            "message": "Ошибка: не удалось получить результат выполнения подкаста. Проверьте настройки и попробуйте еще раз.",
        }
    ...
    if audio is None:
        return {
            ...
            "message": "Ошибка: не удалось сгенерировать аудио-файл для подкаста. Проверьте настройки SALUTE_SPEECH и попробуйте еще раз.",
        }
    
    # Проверяем, что audio является словарем с полем path
    if not isinstance(audio, dict):
        return {
            "transcript": transcript,
            "message": f"Ошибка: неверный формат аудио-файла. Ожидался словарь, получен {type(audio)}.",
        }
    ...
    if not audio_path:
        return {
            "transcript": transcript,
            "message": "Ошибка: аудио-файл не содержит путь. Проверьте настройки загрузки файлов.",
        }
    
    # Преобразуем audio в формат giga_attachments
    # UploadedFile уже содержит все нужные поля: path, size, file_type
    audio_attachment = audio
    
    return {
        "transcript": transcript,
        "message": f'В результате выполнения было сгенерирован аудио-файл {audio_path}. Покажи его пользователю через "![alt-описание](attachment:{audio_path})" и напиши ответ с краткой информацией по подкасту',
        "giga_attachments": [audio_attachment],
    }
