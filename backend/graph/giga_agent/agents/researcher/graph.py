...
- КРИТИЧЕСКИ ВАЖНО: Проверьте, что раздел "Источники" отформатирован как markdown нумерованный список. Каждый источник должен быть на отдельной строке с номером и точкой (например, "1. Название: URL"). Если источники идут сплошным текстом без переносов строк, это критическая ошибка форматирования, которую нужно исправить!"""
...
- Заканчивайте текст заголовком раздела ### Источники. После заголовка перечисляйте источники с соответствующими номерами.
- ВАЖНО: Нумеруйте источники последовательно (1, 2, 3, 4...) в окончательном списке независимо от выбранных источников.
- КРИТИЧЕСКИ ВАЖНО: Источники должны быть отформатированы как markdown нумерованный список. Каждый источник должен быть на ОТДЕЛЬНОЙ строке с номером и точкой в начале. Обязательно добавляйте перенос строки после каждого источника.
- Пример ПРАВИЛЬНОГО формата (каждый источник на отдельной строке):
### Источники
...
- НЕПРАВИЛЬНО (источники в одной строке или без переносов):
### Источники
1. Название источника: URL 2. Название источника: URL 3. Название источника: URL
...
async def researcher_agent(question: str, state: Annotated[dict, InjectedState] = None):
    ...
    ⚠️ КРИТИЧЕСКИ ВАЖНО: Параметр называется question (НЕ query, НЕ user_request, НЕ action)!
    Всегда передавай вопрос/запрос в параметр question.
    
    ПРАВИЛЬНЫЙ ФОРМАТ ВЫЗОВА:
    researcher_agent(question="исследуй тему искусственного интеллекта")
    researcher_agent(question="создай отчет о современных технологиях блокчейн")
    
    НЕПРАВИЛЬНО (НЕ ДЕЛАЙ ТАК):
    ❌ researcher_agent(query="исследуй тему") - параметр query не существует!
    ❌ researcher_agent(user_request="исследуй тему") - параметр user_request не существует!
    ❌ researcher_agent(action="research") - параметр action не существует!
    """
    ...
    # Извлекаем content из последнего сообщения (может быть объект или dict)
    def get_message_content(msg):
        """Безопасно извлекает content из сообщения (объект или dict)"""
    ...
    if "files" in result_state:
        if "final_report.md" in result_state["files"]:
            ...
            final_report = get_message_content(result_state["messages"][-1])
    else:
        final_report = get_message_content(result_state["messages"][-1])
    ...
    # Формируем сообщение с текстом отчета и инструкцией для LLM показать его пользователю
    # Текст отчета должен быть отображен в чате, а файл доступен как вложение
    message_text = f"Исследование завершено. Отчет готов. ОБЯЗАТЕЛЬНО покажи пользователю полный текст отчета ниже:\n\n{final_report}"
    if upload_resp and len(upload_resp) > 0:
        # Формируем рабочую ссылку на файл для пользователя
        # Путь /files/runs/{thread_id}/filename проксируется через nginx
        file_url = f"/files/runs/{thread_id}/research_result.md"
        message_text += f"\n\n---\n\n*Отчёт также сохранён в файле: [{file_url}]({file_url})*"
