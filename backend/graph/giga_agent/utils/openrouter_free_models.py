...
class OpenRouterFreeModels:
    ...
    def __init__(self, api_key: str, config_dir: str = "."):
        ...
        # Примечание: поддерживаем прокси с логином/паролем через AI_PROXY_*.
        ...
        if proxy_url:
            try:
                ...
                # Важно: не логируем proxy_url (в нём могут быть креды).
                http_client = httpx.Client(proxy=proxy_url, timeout=60.0)
            except Exception:
                # Если httpx недоступен/не подходит — fallback: OpenAI сам попробует взять прокси из env.
                http_client = None
        ...
        try:
            ...
            # На случай несовместимой версии openai SDK.
            self.client = OpenAI(
                base_url="https://openrouter.ai/api/v1",
                api_key=api_key,
            )
    ...
    def get_free_models_from_api(self) -> Dict[str, Dict]:
        ...
        try:
            ...
            # Примечание: requests принимает прокси через параметр proxies.
            proxy_url = get_proxy_url_for_http_clients()
    ...
    def get_working_free_models(self) -> List[str]:
        """
        Получение списка рабочих бесплатных моделей
        
        Returns:
            Список идентификаторов рабочих бесплатных моделей, отсортированный по приоритету
        """
        # Получаем все включенные модели
        enabled_models = self.get_enabled_models(sorted_by_size=True)
        
        # Фильтруем только рабочие модели
        working_models = []
        for model_id in enabled_models:
            model_info = self.get_model_info(model_id)
            if model_info.get("working") is True:
                working_models.append(model_id)
        
        # Если нет протестированных рабочих моделей, возвращаем все включенные
        # (они могут быть рабочими, но еще не протестированы)
        if not working_models:
            return enabled_models
        
        return working_models
    
    def get_available_free_models(self, min_context_length: int = 0) -> List[Dict]:
        """
        Получение списка доступных бесплатных моделей с детальной информацией
        
        Args:
            min_context_length: Минимальная длина контекста для фильтрации
        ...
        for model_id in enabled_models:
            ...
            if context_length >= min_context_length:
                available_models.append({
                    ...
                    'working': model_info.get("working", None),
    ...
    def get_detailed_limits(self) -> Dict:
        ...
        Returns:
            Словарь с детальной информацией о лимитах:
            {
                'limit': float или None (если неограничен),
                'limit_remaining': float или None,
                'limit_reset': str (дата сброса),
                'usage': float (все время),
                'usage_daily': float (сегодня),
                'usage_weekly': float (эта неделя),
                'usage_monthly': float (этот месяц),
                'is_free_tier': bool,
                'has_remaining': bool (есть ли остаток лимита)
            }
        """
        ...
        if not limits:
            return {
                ...
                'usage_monthly': 0,
                'is_free_tier': False,
                'has_remaining': True,  # Если не удалось получить, предполагаем что есть
                'error': 'Не удалось получить информацию о лимитах'
            }
        
        limit = limits.get('limit')
        limit_remaining = limits.get('limit_remaining')
        
        # Определяем, есть ли остаток лимита
        has_remaining = True
        if limit is not None:
            if limit_remaining is not None:
                has_remaining = limit_remaining > 0
            else:
                has_remaining = True  # Если остаток не указан, предполагаем что есть
        ...
        return {
            ...
            'usage_daily': limits.get('usage_daily', 0),
            'usage_weekly': limits.get('usage_weekly', 0),
            'usage_monthly': limits.get('usage_monthly', 0),
            'is_free_tier': limits.get('is_free_tier', False),
            'has_remaining': has_remaining
        }
    
    def get_next_available_model(self, current_model: Optional[str] = None, 
                                   exclude_models: Optional[List[str]] = None) -> Optional[str]:
        """
        Получение следующей доступной модели для переключения
        
        Args:
            current_model: Текущая модель (будет исключена из списка)
            exclude_models: Дополнительные модели для исключения
            
        Returns:
            Идентификатор следующей доступной модели или None
        """
        # Получаем рабочие модели
        working_models = self.get_working_free_models()
        
        # Если нет рабочих моделей, используем все включенные
        if not working_models:
            working_models = self.get_enabled_models(sorted_by_size=True)
        
        # Исключаем текущую модель
        if current_model:
            working_models = [m for m in working_models if m != current_model]
        
        # Исключаем дополнительные модели
        if exclude_models:
            working_models = [m for m in working_models if m not in exclude_models]
