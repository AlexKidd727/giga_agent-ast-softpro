...
@contextmanager
def _proxy_env_for_get_urls():
    """
    ВАЖНО (proxy policy):
    Прокси должен применяться ТОЛЬКО для GetUrl/get_urls (Tavily) и OpenAI/OpenRouter.
    Здесь включаем proxy через env ВРЕМЕННО, чтобы langchain_tavily смог его использовать.
    """
...
async def get_urls(urls: list[str], state: Annotated[dict, InjectedState] = None):
    ...
    try:
        # Примечание:
        # TavilyExtract внутри может читать proxy из env. Поэтому включаем proxy временно
        # только на время вызова get_urls.
        with _proxy_env_for_get_urls():
            extract = TavilyExtract()
            response = await extract.ainvoke(
                {"urls": urls, "include_images": False, "extract_depth": "basic"}
            )
        
        # Проверяем тип response - может быть строкой или словарем
        if not response:
            logger.warning(f"Tavily вернул пустой ответ для URLs: {urls}")
            return {
        ...
        # Если response - строка, преобразуем в словарь с результатами
        if isinstance(response, str):
            logger.warning(f"Tavily вернул строку вместо словаря для URLs: {urls}. Ответ: {response[:200]}")
            return {
                "results": [{
                    "error": "Tavily вернул неожиданный формат ответа",
                    "error_type": "UnexpectedResponseFormat",
                    "urls": urls,
                    "images": [],
                    "result": f"Ошибка: сервис вернул неожиданный формат. Ответ: {response[:500]}",
                }],
                "attention": "\nНе удалось обработать ответ от сервиса. Проверьте доступность ссылок.",
            }
        
        # Проверяем наличие результатов в словаре
        if not isinstance(response, dict) or "results" not in response:
            logger.warning(f"Tavily вернул ответ без ключа 'results' для URLs: {urls}. Тип ответа: {type(response)}")
            return {
                "results": [{"error": "Не удалось получить данные со страниц", "urls": urls}],
                "attention": "\nНе удалось получить данные со страниц. Проверьте доступность ссылок.",
            }
        ...
        if not results_list:
            logger.warning(f"Tavily вернул пустой список результатов для URLs: {urls}")
            return {
                "results": [{"error": "Не удалось получить данные со страниц", "urls": urls}],
                "attention": "\nНе удалось получить данные со страниц. Проверьте доступность ссылок.",
            }
        ...
        for result in results_list:
            # Проверяем, что result - словарь
            if not isinstance(result, dict):
                logger.warning(f"Результат от Tavily не является словарем: {type(result)}. Пропускаем.")
                continue
        ...
        # Если нет задач для обработки, возвращаем ошибку
        if not tasks:
            logger.warning(f"Нет валидных результатов для обработки для URLs: {urls}")
            return {
                "results": [{"error": "Не удалось обработать результаты от сервиса", "urls": urls}],
                "attention": "\nНе удалось обработать результаты. Проверьте доступность ссылок.",
            }
